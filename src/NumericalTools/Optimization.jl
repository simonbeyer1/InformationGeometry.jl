...
=======
                store_trace, show_trace, extended_trace, show_every, kwargs...)
        end
    end
    Full ? Res : GetMinimizer(Res)
>>>>>>> Stashed changes
end
...
<<<<<<< Updated upstream
    ...
    Full ? sol : sol.u
=======
    sol = if meth isa Tuple && length(meth) == 2
        Optimization.solve(prob, meth...; maxiters, maxtime, abstol, reltol, kwargs...) # callback
    else
        Optimization.solve(prob, meth; maxiters, maxtime, abstol, reltol, kwargs...) # callback
    end
    if sol.retcode !== ReturnCode.Success 
        verbose && @warn "minimize(): Optimization appears to not have converged."
        if retry
            sol = InformationGeometry.minimize(optf, sol.u, retrymeth; Domain, verbose, tol, maxiters, maxtime, abstol, reltol, lb, lcons, ucons, Full=true, retry=false, kwargs...)
        end
    end
    Full ? sol : GetMinimizer(sol)
>>>>>>> Stashed changes
end
