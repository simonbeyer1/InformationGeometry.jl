...
function minimize(DM::AbstractDataModel, start::AbstractVector{<:Number}=MLE(DM); Lifted::Bool=false, Domain::Union{HyperCube, Nothing}=GetDomain(DM), 
    meth::Union{Nothing, LeastSquaresOptim.AbstractOptimizer, Optim.AbstractOptimizer}=nothing, kwargs...)

    if meth isa LeastSquaresOptim.AbstractOptimizer
        # Call minimizeLeastSquaresOptimJL if meth is a LeastSquaresOptim optimizer
        return minimizeLeastSquaresOptimJL(Data(DM), Predictor(DM), dPredictor(DM), start, nothing, Domain, meth=meth; kwargs...)
    else
        ...
        return isnothing(Cons) ? minimize(F, start, Domain; kwargs...) : minimize(F, start, Domain; lcons=Lcons, ucons=Ucons, cons=Cons, kwargs...)
    end
end


function minimize(DS::AbstractDataSet, Model::ModelOrFunction, start::AbstractVector{<:Number}, LogPriorFn::Union{Nothing, Function}=nothing; 
    Lifted::Bool=false, Domain::Union{HyperCube, Nothing}=GetDomain(Model), meth::Union{Nothing, LeastSquaresOptim.AbstractOptimizer, Optim.AbstractOptimizer}=nothing, kwargs...)
    if meth isa LeastSquaresOptim.AbstractOptimizer
        # Call minimizeLeastSquaresOptimJL if meth is a LeastSquaresOptim optimizer
        return minimizeLeastSquaresOptimJL(DS, Model, dmodel=nothing, start, LogPriorFn, Domain, meth=meth; kwargs...)
    else
        # Otherwise, use the default minimize logic
...
function minimizeLeastSquaresOptimJL(DS::AbstractDataSet, model::ModelOrFunction, dmodel::ModelOrFunction, start::AbstractVector{T}=GetStartP(DS,model), LogPriorFn::Union{Nothing,Function}=nothing, 
    ...
    prob = if isnothing(dmodel)
        LeastSquaresProblem(x = start, f! = model_residual!, output_length = length(ydata(DS)), autodiff = :forward)
    else
        LeastSquaresProblem(x = start, f! = model_residual!, g! = model_jacobian!, output_length = length(ydata(DS)))
    end

    # Use the given optimizer meth: e.g., LevenbergMarquardt(LeastSquaresOptim.LSMR()), Dogleg(LeastSquaresOptim.QR())
    result = optimize!(prob, meth; lower = lb, upper = ub, x_tol = tol, f_tol = tol, g_tol = tol, kwargs...)
